...
function getResultsAutodiagUser(data, callback) {
  ...
  var strsql = ' SELECT ac.id, ac.label, ac.description, ';
      ...
      strsql += ' LEFT JOIN(';
      strsql += ' SELECT id_category, SUM(score) AS score_user, CONCAT("[", GROUP_CONCAT(JSON_OBJECT("answer", aa.label)), "]") AS answers';
      strsql += ' FROM autodiag_answers aa ';
      ...
      strsql += ' LEFT JOIN(';
      strsql += ' SELECT id_category, CONCAT("[", GROUP_CONCAT(JSON_OBJECT("flag", aa.flag)), "]") AS flags';
      strsql += ' FROM autodiag_answers aa ';
      strsql += ' INNER JOIN autodiag_questions aq ON aq.id = aa.id_question ';
      strsql += ' LEFT JOIN autodiag_user_answers aua ON aua.id_answer = aa.id AND aua.id_user = ' + data.id;
      strsql += ' WHERE aa.deleted IS NULL AND aq.deleted IS NULL AND aa.flag IS NOT NULL AND (aa.revert IS NULL AND aua.id IS NOT NULL OR aa.revert IS NOT NULL AND aua.id IS NULL)';
      strsql += ' GROUP BY id_category';
      strsql += ' )Flags_User ON Flags_User.id_category = ac.id';
